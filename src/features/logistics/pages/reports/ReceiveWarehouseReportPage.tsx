...
export const ReceiveWarehouseReportPage: FC = () => {
  ...
  const handleSubmit = useCallback(() => {
    setParams({
      serviceName: viewMode === 'grid' ? 'olapReceivedTotalReport' : 'olapChartColTopReceivePurchase',
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1, overflow: 'auto' }}>
      <PageHeader breadcrumbs={[{ label: 'Logistics' }, { label: 'Báo cáo', path: '/logistics/reports' }, { label: 'Nhập kho' }]} />
      <Box sx={{ px: 2, pb: 2 }}>
        ...
        <Paper sx={{ p: 2 }}>
          <Typography variant="subtitle2" sx={{ mb: 1 }}>
            {viewMode === 'grid' ? 'Chi tiết nhập kho' : 'Top sản phẩm nhập kho'}
          </Typography>
