...
export const InventoryReportPage: FC = () => {
  ...
  const handleSubmit = useCallback(() => {
    setParams({
      serviceName: viewMode === 'grid' ? 'olapInventoryReport' : 'olapInventoryProductTotal',
      ...
      dateType,
      filterTop: viewMode === 'chart' ? '20' : undefined,
      filterSort: viewMode === 'chart' ? 'EXPORT_DESC' : undefined,
    });
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1, overflow: 'auto' }}>
      <PageHeader breadcrumbs={[{ label: 'Logistics' }, { label: 'Báo cáo', path: '/logistics/reports' }, { label: 'Tồn kho' }]} />
      <Box sx={{ px: 2, pb: 2 }}>
        ...
        <Paper sx={{ p: 2 }}>
          <Typography variant="subtitle2" sx={{ mb: 1 }}>
            {viewMode === 'grid' ? 'Tổng hợp tồn kho' : 'Top sản phẩm xuất/nhập'}
          </Typography>
