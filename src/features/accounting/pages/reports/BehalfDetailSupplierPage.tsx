...
const columns: Column<Row>[] = [
  ...
  { key: 'transId', label: 'Mã GD', width: 100 },
...
export const BehalfDetailSupplierPage: FC = () => {
  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(20);

  const { data, isLoading } = useQuery({
    queryKey: ['behalf-detail-supplier', page, pageSize],
    queryFn: () => accReportApi.listPayColBehalfDetailByTrans(page, pageSize),
  });

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1, overflow: 'auto' }}>
      <PageHeader breadcrumbs={[{ label: 'Kế toán' }, { label: 'Báo cáo', path: '/accounting/reports' }, { label: 'Chi tiết thu hộ (NCC)' }]} />
      <Box sx={{ px: 2, pb: 2, flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <Paper sx={{ p: 2, mb: 1 }}>
          <Typography variant="subtitle2">Chi tiết thu hộ theo giao dịch (NCC)</Typography>
        </Paper>
